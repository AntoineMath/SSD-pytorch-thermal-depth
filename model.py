...
class VGGBaseAfterMerge(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv2_1 = nn.Conv2d(128, 128, kernel_size=3, padding=1) # changed input to 128 since we merge botch features before
    ...
    def forward(self, out_pool1):
        ...
        :param out_pool1: (N, 64, 150, 150)
        :return: lower-level feature maps conv4_3 and conv7
        """
        out = F.relu(self.conv2_1(out_pool1))  # (N, 128, 150, 150)
...
class SSD300(nn.Module):
    ...
    def forward(self, thermal_image, depth_image):
        """
        Forward propagation.
        ...
        pool1_thermal_feats = self.base_pre_merge_thermal(thermal_image)  # (N, 64, 150, 150)
        pool1_depth_feats = self.base_pre_merge_depth(thermal_image)  # (N, 64, 150, 150)
        pool1_merged = torch.cat((pool1_thermal_feats, pool1_depth_feats), axis=1)  # (N, 128, 150, 150

        # Run the base_after_merge network on the merged features
        conv4_3_feats, conv7_feats = self.base_after_merge(pool1_merged)
        ...
        for i in range(batch_size):
            ...
            decoded_locs = cxcy_to_xy(
                gcxgcy_to_cxcy(predicted_locs[i],
                               self.priors_cxcy.to(device)))  # (8732, 4), these are fractional pt. coordinates
